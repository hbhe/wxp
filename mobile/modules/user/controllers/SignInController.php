...
namespace mobile\modules\user\controllers;

use common\commands\SendEmailCommand;
use common\models\User;
use common\models\UserToken;
use mobile\modules\user\models\LoginForm;
use mobile\modules\user\models\PasswordResetRequestForm;
use mobile\modules\user\models\ResetPasswordForm;
use mobile\modules\user\models\SignupForm;
use Yii;
 ...
 * @package mobile\modules\user\controllers
...
class SignInController extends \yii\web\Controller
{
    ...
    public function actionActivation($token)
    {
        ...
        Yii::$app->getUser()->login($user);
        Yii::$app->getSession()->setFlash('alert', [
            'body' => Yii::t('mobile', 'Your account has been successfully activated.'),
            'options' => ['class'=>'alert-success']
        ]);
    ...
    public function actionRequestPasswordReset()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            if ($model->sendEmail()) {
                Yii::$app->getSession()->setFlash('alert', [
                    'body'=>Yii::t('mobile', 'Check your email for further instructions.'),
                    'options'=>['class'=>'alert-success']
                ]);
                ...
                Yii::$app->getSession()->setFlash('alert', [
                    'body'=>Yii::t('mobile', 'Sorry, we are unable to reset password for email provided.'),
    ...
    public function actionResetPassword($token)
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate() && $model->resetPassword()) {
            Yii::$app->getSession()->setFlash('alert', [
                'body'=> Yii::t('mobile', 'New password was saved.'),
                'options'=>['class'=>'alert-success']
            ]);
            return $this->goHome();
        }
        ...
        if (!$user) {
            ...
            if ($user->save()) {
                ...
                $sentSuccess = Yii::$app->commandBus->handle(new SendEmailCommand([
                    ...
                    'subject' => Yii::t('mobile', '{app-name} | Your login information', ['app-name'=>Yii::$app->name]),
                    'to' => $user->email
                ]));
                if ($sentSuccess) {
                    Yii::$app->session->setFlash(
                        ...
                        [
                            'options'=>['class'=>'alert-success'],
                            'body'=>Yii::t('mobile', 'Welcome to {app-name}. Email with your login information was sent to your email.', [
                ...
                if ($user->email && User::find()->where(['email'=>$user->email])->count()) {
                    Yii::$app->session->setFlash(
                        ...
                        [
                            'options'=>['class'=>'alert-danger'],
                            'body'=>Yii::t('mobile', 'We already have a user with email {email}', [
                                'email'=>$user->email
                            ])
                        ]
                    );
                    ...
                    Yii::$app->session->setFlash(
                        ...
                        [
                            'options'=>['class'=>'alert-danger'],
                            'body'=>Yii::t('mobile', 'Error while oauth process.')
                        ]
                    );
                }
